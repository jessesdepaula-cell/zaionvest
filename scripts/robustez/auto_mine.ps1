...
do {
    ...
    foreach ($Ativo in $Ativos) {
        foreach ($Tf in $Timeframes) {
            ...
            # 1. Roda a evolucao genetica com holdout OOS (usando -u para saida nao-bufferizada)
            & $PythonExe -u scripts/robustez/genetic.py --symbols $Ativo --timeframes $Tf --pop 80 --gen 30 --keep 12 --out $OutFile
            ...
            if (Test-Path $OutFile) {
                ...
                & $PythonExe -u scripts/robustez/publish.py --survivors $OutFile
